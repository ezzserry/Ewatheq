...
import ae.ewatheq.ocr.language.LanguageCodeHelper;
import ae.ewatheq.ocr.language.TranslateAsyncTask;
 ...
 */
public final class OCRActivity extends Activity implements SurfaceHolder.Callback, 
ShutterButton.OnShutterButtonListener {
	...
	public static final String DEFAULT_PAGE_SEGMENTATION_MODE = "Sparse text";
	...
	///Noman : Testing 
	private static final String STR_RECOGNITION_ERROR= "Not Recognizable";
	private static final String STR_WRONG_COUNTRY= "wrong country";
	public static final String STR_OCR_DATA_NAME = "ocr_data_name";
	public static final String STR_OCR_DATA_EMIRATES_ID = "ocr_data_emirates_id";
	public static final String STR_OCR_DATA_COUNTRY = "ocr_data_country";
	public static final String STR_OCR_DATA_DATE_OF_BIRTH = "ocr_data_dob";
	public static final String STR_OCR_DATA_GENDER = "ocr_data_gender";
	public static final String STR_OCR_DATA_ERROR = "ocr_data_error";
	public static final String STR_OCR_DATA_ERROR_REASON_UAE_NATIONALS = "Only UAE nationals can register to this application";

	private static final String UAE_NATIONAL_STRING = "IDARE";
	private static final String NON_UAE_NATIONAL_STRING = "ILARE";
	...
	public void onCreate(Bundle icicle) {
		...
		String strLangId = EWatheqUtils.getPrefString(
				activity,
				Constants.LANGUAGES_IDS);
		TextView tvTitle = (TextView)findViewById(R.id.tv_top_header_title);
		if (strLangId.equalsIgnoreCase(""+Constants.LANGUAGE_ID_AR))
		{

			if(tvTitle != null)
			{
				tvTitle.setText(getString(R.string.str_please_scan_back_of_id_card_ar));
			}
		}
		else
		{
			if(tvTitle != null)
			{
				tvTitle.setText(getString(R.string.str_please_scan_back_of_id_card_en));
			}
		}
	...
	private void initCamera(SurfaceHolder surfaceHolder) {
		...
		try {
			...
			showErrorMessage("Error", "IOException - Could not initialize camera. Please try restarting device.");
		} catch (RuntimeException e) {
			// Barcode Scanner has seen crashes in the wild of this variety:
			// java.?lang.?RuntimeException: Fail to connect to camera service
			showErrorMessage("Error", "RuntimeException - Could not initialize camera. Please try restarting device.");
		}   
	}
	...
	boolean handleOcrDecode(OcrResult ocrResult) {
		...
		setCardTitles(ocrResultView, ocrResult.getText());
	...
	public void setCardTitles(View resultView, String resultText)
	{

		TextView ocrResultTextViewName = (TextView) findViewById(R.id.ocr_result_text_view_name);
		TextView ocrResultTextViewCountry = (TextView) findViewById(R.id.ocr_result_text_view_country);
		TextView ocrResultTextViewId = (TextView) findViewById(R.id.ocr_result_text_view_id);
		String name = "",id = "",country = "";
		String []resulTextRows = resultText.split("\n");
		int textRowsCount = 0; 
		if (resulTextRows.length >= 3)
		{

			for (int j=textRowsCount; j<resulTextRows.length;j++)
			{
				if (resulTextRows[textRowsCount].length()<20){
					textRowsCount++;
				}
				else 
				{
			...
			if (resulTextRows[textRowsCount].length()>15)
			{
				id = resulTextRows[textRowsCount].substring(resulTextRows[textRowsCount].length()-15, resulTextRows[textRowsCount].length());
			}
			...
			if (resulTextRows[textRowsCount].length()>15)
			{
				int firstArrowPositiioin = resulTextRows[textRowsCount].indexOf("<");
				country = resulTextRows[textRowsCount].substring(firstArrowPositiioin-3, firstArrowPositiioin);
			}
			textRowsCount++;
			for (int j=textRowsCount; j<resulTextRows.length;j++)
			{
			...
			String nameParts[] = resulTextRows[textRowsCount].split("<") ;
		...
		ocrResultTextViewCountry.setText(country);
		ocrResultTextViewId.setText(id);
		ocrResultTextViewName.setText(name);
	...
	void handleOcrContinuousDecode(OcrResult ocrResult) {
		...
		if (CONTINUOUS_DISPLAY_RECOGNIZED_TEXT) {
			statusViewTop.setText("Scanning ...");
			statusViewTop.setVisibility(View.VISIBLE);
			//validateAndShowOCRResult(ocrResult.getText());
			checkIfUAENationalOrNot(ocrResult.getText());
		}
	...
	//Noman : Testing showing ocr result 
	public boolean validateAndShowOCRResult(String resultText)
	{
		...
		if (resultText != null && resultText.contains(" "))
		{
			...
			String []resulTextRows = resultText.split("\n");
			String []actualResulTextRows = new String [3];
			if (resulTextRows.length >= 3)
			{
				int j = 0;
				for(int i = 0;i <resulTextRows.length;i++)
				{
					if (resulTextRows[i].length()>=26)
					{
						actualResulTextRows[j] =resulTextRows[i];
						j++;
						if (j==3)
							break;
					}
				...
				if (j == 3)
				{
					//showOCRResult(actualResulTextRows);
					returnOCRResult(actualResulTextRows);
					return true;
				}
				else 
					return false;
	...
	public String [] getAllScannedRows(String result)
	{
		String []resulTextRows = result.split("\n");
		String []actualResulTextRows = {"", "", ""};
		if (resulTextRows.length != 3)
		{
			int j = 0;
			for(int i = 0;i <resulTextRows.length;i++)
			...
			if (j != 3 )
			{
				actualResulTextRows[0]= "";
				actualResulTextRows[1]= "";
				actualResulTextRows[2]= "";
			}

		}
		else 
			actualResulTextRows = resulTextRows;
		return actualResulTextRows;
	}
	public String getIdRow(String result)
	{
	
		String idRow = "";
		Log.e("OCR", "Start IDROW :"+result );
		try {
		String []resulTextRows = result.split("\n");
		if (resulTextRows.length>=3)
		{
			if(resulTextRows[0].length()>18)
				idRow = resulTextRows[0];
			else 
			{
				idRow = resulTextRows[0];
				String idRowSecondPart = "";
				for(int i=1;i<resulTextRows.length;i++)
				{
					if (resulTextRows[i].length()>10)
					{
						idRowSecondPart = resulTextRows[i];
						break;
					}
				}
				idRow = idRow+idRowSecondPart;
			}
		}
		}
		catch (Exception ex)
		...
		Log.e("OCR", "End IDROW :"+idRow );
		return idRow;
	}
	public String getDobRow(String result)
	{
		Log.e("OCR", "Start DOBROW :"+result );

		String dobRow = "";
		try{
			String []resulTextRows = result.split("\n");
			if (resulTextRows.length>=2)
			{
				for(int i=0;i<resulTextRows.length;i++)
				{
					if(resulTextRows[i].length()>7)
					{
						dobRow = resulTextRows[i];
						break;
					}
		...
		Log.e("OCR", "End DOBROW :"+dobRow );

		return dobRow;
	}
	public String getNameRow(String result)
	{
		Log.e("OCR", "Start NAMEROW :"+result );
		String nameRow = "";
		try{
			String []resulTextRows = result.split("\n");
			...
			if (resulTextRows.length>=3)
			{
				for(int i=resulTextRows.length-1;i>=0;i--)
				{
					if(resulTextRows[i].length()>15)
					{
						nameRow = resulTextRows[i];
						break;
					}
				}
			}
		...
		Log.e("OCR", "End NAMEROW :"+nameRow );
		return nameRow;
	}
	public boolean checkIfUAENationalOrNot(String result)
	{


		if (result != null && result.contains(" "))
			result = result.replace(" ", "");
		if (result.contains(UAE_NATIONAL_STRING))
		{
			returnSuccessOCRResult(result.substring(result.indexOf(UAE_NATIONAL_STRING)));
			return true;
		}
...
//			returnFailureOCRResult();
	...
	public void returnSuccessOCRResult(String result)
	{
		String idRow = getIdRow(result);
		String nameRow = getNameRow(result);
		String dobRow = "";
		String id =  "";
		String name =  "";
		String dob =  "";
		String gender =  "";
		if (idRow!= null && idRow.length()>21)
		{
			id = idRow.substring(idRow.length()-12);
			dobRow =getDobRow(result.substring(result.indexOf(id)+13));
			id = "784"+id;
		}
		if (!getRecognizedId(id).equalsIgnoreCase(STR_RECOGNITION_ERROR))
		{
			name = getName(nameRow);
			if (dobRow!= null &&dobRow.length()>=8){
				dob = dobRow.substring(0,6);
				gender = dobRow.substring(7,8);
			}
			sendResultBack(name, id, "ARE", dob, gender);
		}

	}
	public void sendResultBack (String name,String emId,String mCountry, String dob, String gender)
	{
		Intent  result = new Intent();
		result.putExtra(OCRActivity.STR_OCR_DATA_NAME, name);
		result.putExtra(OCRActivity.STR_OCR_DATA_EMIRATES_ID,emId);
		result.putExtra(OCRActivity.STR_OCR_DATA_COUNTRY,mCountry);
		result.putExtra(OCRActivity.STR_OCR_DATA_DATE_OF_BIRTH,getRecognizedDOB(dob));
		result.putExtra(OCRActivity.STR_OCR_DATA_GENDER,getRecognizedGender(gender ));

		setResult(Activity.RESULT_OK,result);
		finish();

	}
	public String getName (String  nameRow)
	{
		String name = "";
		if (nameRow!= null){
			String[] nameArray = nameRow.split("<");
		...
		return getRecognizedName(name);
	}
	public void returnFailureOCRResult()
	{
		Intent result = new Intent();
		result.putExtra(OCRActivity.STR_OCR_DATA_ERROR,OCRActivity.STR_OCR_DATA_ERROR_REASON_UAE_NATIONALS);
		setResult(Activity.RESULT_CANCELED,result);
		finish();
	}
	public void returnOCRResult (String[] ocrResultRows)
	{
		getTheOCRReturnIntent(ocrResultRows);

	}
	public void showOCRResult (String[] ocrResultRows)
	{
		if (alertDialog == null || !alertDialog.isShowing())
		{
			String dialogMessage = getTheOCRString(ocrResultRows);
			if (dialogMessage != null && dialogMessage.length()>0)
			{
				alertDialog = new AlertDialog.Builder(OCRActivity.this).create();
				alertDialog.setTitle("OCR Result");
	...
	public String getTheOCRString(String[] ocrResultRows)
	{
		for (int i=0;i<ocrResultRows.length;i++)
		{
			if (ocrResultRows[i].contains(" "))
				ocrResultRows[i]=ocrResultRows[i].replace(" ", "");
		}
		String country="",id="",dob="",gender="",name="",result = "";
		try {
			id = ocrResultRows[0].substring(ocrResultRows[0].length()-15,ocrResultRows[0].length());
			int firstArrowPosition = ocrResultRows[1].indexOf("<");
			if (firstArrowPosition != -1)
				country = ocrResultRows[1].substring(ocrResultRows[1].indexOf("<")-3,ocrResultRows[1].indexOf("<"));
			dob = ocrResultRows[1].substring(0,6);
			gender = ocrResultRows[1].substring(7,8);
			String[] nameArray = ocrResultRows[2].split("<");
		...
		if (!getRecognizedId(id).equalsIgnoreCase(STR_RECOGNITION_ERROR) &&
				!getRecognizedCountry(country).equalsIgnoreCase(STR_RECOGNITION_ERROR)	)
		{
			result =   		  "Name    : " +  getRecognizedName(name) +"\n";
			result = result + "ID      : " + getRecognizedId(id) +"\n";
			result = result + "Country : " + getRecognizedCountry(country) +"\n";
			result = result + "DOB     : " + getRecognizedDOB(dob )+"\n";
			result = result + "Gender  : " + getRecognizedGender(gender )+"\n";
		}
		return result;
	}
	public void getTheOCRReturnIntent(String[] ocrResultRows)
	{
		for (int i=0;i<ocrResultRows.length;i++)
		{
			if (ocrResultRows[i].contains(" "))
		...
		String country="",id="",dob="",gender="",name="";
		Intent result = null;
		try {
			id = ocrResultRows[0].substring(ocrResultRows[0].length()-15,ocrResultRows[0].length());
			int firstArrowPosition = ocrResultRows[1].indexOf("<");
		...
		String mCountry = getRecognizedCountry(country);
		String mId = getRecognizedId(id);

		if (!getRecognizedId(id).equalsIgnoreCase(STR_RECOGNITION_ERROR) &&
				!mCountry.equalsIgnoreCase(STR_RECOGNITION_ERROR) 
				&& !mCountry.equalsIgnoreCase(STR_WRONG_COUNTRY)	)
		{
			result = new Intent();
			result.putExtra(OCRActivity.STR_OCR_DATA_NAME, getRecognizedName(name));
			result.putExtra(OCRActivity.STR_OCR_DATA_EMIRATES_ID,mId);
			result.putExtra(OCRActivity.STR_OCR_DATA_COUNTRY,mCountry);
			result.putExtra(OCRActivity.STR_OCR_DATA_DATE_OF_BIRTH,getRecognizedDOB(dob ));
			result.putExtra(OCRActivity.STR_OCR_DATA_GENDER,getRecognizedGender(gender ));
		...
		else if (mCountry.equalsIgnoreCase(STR_WRONG_COUNTRY) && isAnotherCountry(activity, country))
		{
			result = new Intent();
			result.putExtra(OCRActivity.STR_OCR_DATA_ERROR,OCRActivity.STR_OCR_DATA_ERROR_REASON_UAE_NATIONALS);
			setResult(Activity.RESULT_CANCELED,result);
	...
	public static String getRecognizedName(String name)
	{
		if (isName(name))
			return name;
		else 
	...
	public static String getRecognizedId(String id)
	{
		if (isId(id))
			return id;
		else 
			return STR_RECOGNITION_ERROR;
	}	
	public static String getRecognizedCountry(String country)
	{
		/*	if (isCountryCriteriaFilled(country) && isCountryUAE(country))
			return country;
		else if (!isCountryCriteriaFilled(country))
			return STR_RECOGNITION_ERROR;
		else
			return STR_WRONG_COUNTRY;*/
		if ( isCountryUAE(country))
			return country;
		/*else if (!isCountryCriteriaFilled(country))
			return STR_RECOGNITION_ERROR;*/
		else
	...
	public static boolean isAnotherCountry(Context context,String country)
	{
		String countries[] = context.getResources().getStringArray(R.array.iso_country_codes);
		for (int i=0; i<countries.length;i++)
		{
			int comp = countries[i].compareToIgnoreCase(country);
	...
	public static String getRecognizedDOB(String dob)
	{
		if (isDOB(dob))
			return dob;
		else 
	...
	public static String getRecognizedGender(String gender)
	{
		if (isGender(gender))
			return gender;
		else 
	...
	public static boolean isName(String var) {
		String capitalAlphabetPattern = "[A-Z ]+";
		return var.matches(capitalAlphabetPattern) ;
	}
	public static boolean isId(String var) {
		Log.e("OCR", "ID Recognition :" + var);
		return var.matches("[0-9]+")&& var.length() == 15 && isIdByStandard(var);
	}
	/*Noman : Testing (Id checking as written in https://en.wikipedia.org/wiki/National_identification_number
	 * under UAE heading) 
	 */

	public static boolean isIdByStandard(String var) {

		int intIdIsoNumeric = Integer.parseInt(var.substring(0,3));
		int intIdIsoDobYear = Integer.parseInt(var.substring(3,7));
		if (intIdIsoNumeric == 784 && intIdIsoDobYear > 1900 && intIdIsoDobYear < 2015)
			return true;
		return false;

	}
	public static boolean isDOB(String var) {
		return var.matches("[0-9]+")&& var.length() == 6;
	}
	public static boolean isGender(String var) {
		return var.matches("[MF]+")&& var.length() == 1;
	}
	public static boolean isCountryCriteriaFilled(String var) {
		return var.matches("[A-Z]+")&& var.length() == 3;
	}
	public static boolean isCountryUAE(String var) {
		String first2Characters = var.substring(0,2);
		String last2Characters = var.substring(1);
		if (var.equalsIgnoreCase("ARE"))
			return true;
		if (first2Characters.equalsIgnoreCase("AR")&&
				!var.equalsIgnoreCase("ARG")&&
				!var.equalsIgnoreCase("ARM")&&
				!var.equalsIgnoreCase("MAR"))
		{
			return true;
		}
		if (last2Characters.equalsIgnoreCase("RE")&&
				!var.equalsIgnoreCase("REU"))
		{
			return true;
		}
	...
	private void requestDelayedAutoFocus() {
		...
		cameraManager.requestAutoFocus(35L);
	}
	...
	private void retrievePreferences() {
		...
		PreferenceManager.setDefaultValues(this, R.xml.ocr_preferences, false);
	...
	private void setDefaultPreferences() {
		...
		prefs.edit().putString(PreferencesActivity.KEY_CHARACTER_BLACKLIST, 
				OcrCharacterHelper.getDefaultBlacklist(OCRActivity.DEFAULT_SOURCE_LANGUAGE_CODE)).commit();

		// Character whitelist
		prefs.edit().putString(PreferencesActivity.KEY_CHARACTER_WHITELIST, 
				OcrCharacterHelper.getDefaultWhitelist(OCRActivity.DEFAULT_SOURCE_LANGUAGE_CODE)).commit();
