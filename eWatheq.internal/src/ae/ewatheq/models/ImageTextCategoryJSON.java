...
public class ImageTextCategoryJSON implements Serializable{
	...
	public static ImageTextCategoryJSON parse(JSONObject jSonObj)
	{
		ImageTextCategoryJSON catWBooks = new ImageTextCategoryJSON();
		try {
	...
	public static List<ImageTextCategoryJSON> parse(JSONArray jSonarray)
	{
		List<ImageTextCategoryJSON> catWBooks = new ArrayList<ImageTextCategoryJSON>();
		for(int i=0;i<jSonarray.length();i++)
		{
			try {
				ImageTextCategoryJSON cat = ImageTextCategoryJSON.parse(jSonarray.getJSONObject(i));
				catWBooks.add(cat);
			} catch (Exception e) {
